...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
@interface NIPhotoScrollView() <MBProgressHUDDelegate>
...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    _imageView = [[BaseImageView alloc] initWithFrame:CGRectZero];
...
- (void)didDoubleTap:(UITapGestureRecognizer *)tapGesture {
  ...
  if (isCompletelyZoomedIn) {
    // Zoom the photo back out.
      ...
      float zoomScale = _scrollView.bounds.size.width / self.photoDimensions.width;
...
//    //added by hanchao
//    if (photoSize == NIPhotoScrollViewPhotoSizeThumbnail) {
//        [self showIndicator];
//    }else{
//        [self hideIndicator];
//    }
...
//#pragma added by hanchao
//-(void)showIndicator
//{
//    if (!_progressBar) {
//        _progressBar = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
//        _progressBar.frame = CGRectMake((self.frame.size.width-_progressBar.frame.size.width)/2,
//                                        (self.frame.size.height-_progressBar.frame.size.height)/2,
//                                        _progressBar.frame.size.width,
//                                        _progressBar.frame.size.height);
//        _progressBar.hidesWhenStopped = YES;
...
//  _scrollView.minimumZoomScale = minScale;
    ...
    /*
     * modified by hanchao
     * 当缩略图时，图片缩放至屏幕宽度，显示左上角图片
     */
    float scale = boundsSize.width / self.photoDimensions.width;
    ...
    if (isnan(scale) || isinf(scale)) {
        ...
        if([self isZoomingEnabled]){
            minScale = MIN(1.0f,scale);
            maxScale = MAX(2 * scale,2);
            
        }else{
            if (self.photoSize == NIPhotoScrollViewPhotoSizeUnknown) {
                scale = 1;
            }
    ...
    _scrollView.zoomScale = scale;//_scrollView.minimumZoomScale;
